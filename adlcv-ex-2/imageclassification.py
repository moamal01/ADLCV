...
def main(image_size=(32,32), patch_size=(4,4), channels=3, 
         ...
         pos_enc='learnable', pool='cls', dropout=0.3, fc_dim=1024, 
    ...
    cwd = os.getcwd()
    model_path = os.path.join(cwd,'models', 'model5.pth')
    print(f'Model specifications: num_heads {num_heads}\tnum_layers {num_layers}\tfc_dim {fc_dim}\tpathsize{patch_size}\tdropout{dropout}')
    ...
    for e in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_loss <= best_val_loss:
                torch.save(model.state_dict(), os.path.join(cwd,'models', 'model.pth'))
